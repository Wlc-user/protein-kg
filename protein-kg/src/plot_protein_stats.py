"""
蛋白质数据基础可视化（适配 gzip 压缩的 FASTA 和实际 JSON 结构：nodes + relations）
"""
import json
import gzip
...
# 1. 读取 FASTA 序列（支持 gzip 压缩）
print("读取 FASTA 序列...")
fasta_path = DATA_DIR / "human_proteome.fasta"
seq_lengths = []

# 检测是否为 gzip 压缩（前两个字节是 0x1f 0x8b）
...
nodes = kg.get("nodes", [])
relations = kg.get("relations", [])   # 注意：你的 JSON 中是 "relations"，不是 "edges"
